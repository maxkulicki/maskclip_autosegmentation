...
class ClipCaptionModel(nn.Module):
    ...
    def get_dummy_token(self, batch_size: int):
    ...
    def __init__(self,  device, prefix_length: int, prefix_size: int = 512):
...
def init_clip_cap(model_path, device):
    prefix_length = 10
    model = ClipCaptionModel(device, prefix_length)
    model.load_state_dict(torch.load(model_path))
    tokenizer = GPT2Tokenizer.from_pretrained("gpt2")
    model.to(device)
    return model, tokenizer
def caption_from_CLIP(embedding, model, tokenizer, beam_search=True):
    prefix_length = 10
