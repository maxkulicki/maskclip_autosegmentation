...
for i in segments_ids:
    elements = np.nonzero(segments==i)
    segment_info.append({
        'neighbors': set(),
        'elements': elements,
        'size': len(elements),
        'avg_embeddding': np.zeros(512)
    })

for pair in bneighbors.T:
    n1, n2 = pair
    if n1 != n2:
        segment_info[n1-1]['neighbors'].add(n2)
...
# ### 3. Run program
#
# imagePath = 'demo/bear.jpg'
#
# #SLIC Parameters:
# numSegments = 100  # How many superpixels to start with - input for SLIC function
# Sigma = 4 # This parameter controls superpixel shape. Higher values make superpixels more square.
#
# #SPHC Parameters:
# segmentsToMerge = 100 # How many superpixels to merge based on color similarity
# distance_limit = .2  # Limits which segments get merged based on their difference in average color
# ####################################################################################################
#
...
#         merge_count += 1
#         if merge_count % 20 == 0:
#             print(f"{merge_count} segments merged")
#
#     print(f"{merge_count} segments merged - final")
#
